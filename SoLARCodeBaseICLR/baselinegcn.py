...
gcn_start = time.time()
finaltestaccafter,_,_ = train_and_get_results(data, model,lr)
gcn_end = time.time()
...
headers = ['Model','Dataset','AvgTestAccAfter','DeviationAfter',
        'HiddenDim','LR','Dropout','TrainNodes','TestNodes','ValNodes','EdgesAdded','EdgesRemoved','GCNTime']
...
with open(filename, mode='a', newline='') as file:
    ...
    writer.writerow([args.model, args.dataset, f"{(avg_test_acc_after):.4f}", f"{(std_dev_after):.4f}",
                     hidden_dimension, lr, p, num_train_nodes/100,num_test_nodes/100,num_val_nodes/100, EdgesAdded,EdgesRemoved, gcn_end - gcn_start])
