...
def get_graph_and_labels_from_pyg_dataset(dataset):
    ...
    graph.add_edges_from(dataset.edge_index.cpu().T.numpy())
...
def count_edges(data):
    ...
    return inter_class_edges, intra_class_edges




def uncertainty_coefficient(L, C):
    """
    Calculate the uncertainty coefficient U(L|C) = I(L;C)/H(L)
    where I is the mutual information and H is the entropy.
    
    Parameters:
    L (list): Labels of nodes
    C (list): Communities of nodes

    Returns:
    float: The uncertainty coefficient
    """
    # Calculate the mutual information I(L;C)
    I = mutual_info_score(L, C)

    # Calculate the entropy H(L)
    H = entropy(np.bincount(L))

    # Calculate the uncertainty coefficient U(L|C)
    U = I / H if H else 0
